...
struct Solver {
    ...
    Solver() {
        ...
        priority_queue<pair<int, T>, vector<pair<int, T>>, greater<pair<int, T>>> pq1;
        priority_queue<pair<int, P>, vector<pair<int, P>>, greater<pair<int, P>>> pq2;
        ...
        for(int x=1; x<=n; x++) for(auto y : cross_points_row[x]) {
            ...
            swap(tmp_dist, dist_from_to[x][y]);
        }
        // cerr << "road num: " << road_num << endl;
        return;
    }
        ...
        while(flag && !beam.empty()) {
            ...
            while(!beam.empty()) {
                ...
                for(int dir=0; dir<DIR_NUM; dir++) {
                    ...
                    // 全ての道路を通った時
                    if(next_visited.count() == road_num-1 && best_score < next_score) {
